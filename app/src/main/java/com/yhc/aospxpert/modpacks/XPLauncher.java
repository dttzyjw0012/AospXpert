...
import static com.yhc.aospxpert.BuildConfig.APPLICATION_ID;
...
import static com.yhc.aospxpert.modpacks.utils.BootLoopProtector.isBootLooped;
...
public class XPLauncher implements ServiceConnection {
	...
	private void onXPrefsReady(XC_LoadPackage.LoadPackageParam lpParam) {
		if (isBootLooped(lpParam.packageName)) {
			log(String.format("AospXpert: Possible bootloop in %s. Will not load for now", lpParam.packageName));
			return;
		}
	...
	private void waitForXprefsLoad(XC_LoadPackage.LoadPackageParam lpParam) {
		...
		log("AospXpert Version: " + BuildConfig.VERSION_NAME);
		try {
			log("AospXpert Records: " + Xprefs.getAll().keySet().size());
		} catch (Throwable ignored) {
		}
